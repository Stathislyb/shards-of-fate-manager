...
<script>
    export default {
        data() {
            ...
            if (this.$store.state.pages.session_list_delete) 
            {
                state = this.$store.state.pages.session_list_delete;
            }
                        ...
                        render: (h, params) => {
                            ...
                            return h('div', [
                                h('Button', {
                                    ...
                                    on: {
                                        click: () => {
                                            this.$router.push({ name: 'session_update', params: { id: row.id } });
                                        }
                                    }
                                }, 'Edit'),
                                h('Button', {
        ...
        watch: {
            formValidate: {
                ...
                handler(value) 
                {
                    this.$store.commit('pages/session_list_delete/setFormValidate', value);
                }
            }
        },
        methods: {
            ajax() {
                ...
                return {
                    ...
                    delete(id) {
                        window.axios.delete( process.env.MIX_BASE_RELATIVE_URL_BACKEND+'/trident/resource/Session/'+id ).then(({ data }) => {
                            // console.log(data);
            ...
            on_create_button_clicked() {
                this.$router.push({ name: 'session_create' });
            },
        },
        mounted() {
