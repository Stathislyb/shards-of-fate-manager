...
<script>
    export default {
        data() {
            ...
            if (this.$store.state.pages.session_create) 
            {
                state = this.$store.state.pages.session_create;
            }
        ...
        watch: {
            formValidate: {
                ...
                handler(value) 
                {
                    this.$store.commit('pages/session_create/setFormValidate', value);
                }
            }
        },
        methods: {
            ajax() {
                ...
                return {
                    create(data) {
                        ...
                        window.axios.post( process.env.MIX_BASE_RELATIVE_URL_BACKEND+'/trident/resource/Session',  form_data ).then((response) => {
